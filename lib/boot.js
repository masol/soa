...
const defPlugins = ['cors', 'circuit-breaker', 'accepts', 'compress', 'static', 'session', 'formbody', 'multipart', 'objection', 'auth']
const defPkgs = []

async function setup (fastify, opts) {
  const { soa, _, $, config } = fastify
  const cfgutil = config.util
  const allServices = _.keys(config)
  ...
  await Promise.all([
    ...
    $.map(defPlugins, async (plugName) => {
      ...
      if (!config.has(plugName)) { // config中没有定义此服务，因此没有禁止。
